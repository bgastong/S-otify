...
function Header({
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      try {
        const songs = await songsService.getSongs({ page: 1, limit: 10 });
        const uniqueGenres = [
      ...
      >
        ...
        >
          <div className="w-6 h-5 flex flex-col justify-between items-end">
            <span
              className={`h-0.5 bg-white/80 rounded-full transition-all ${isMenuOpen ? "w-6 rotate-45 translate-y-2" : "w-6"}`}
            ></span>
            <span
              className={`h-0.5 bg-white/80 rounded-full transition-all ${isMenuOpen ? "opacity-0" : "w-4"}`}
            ></span>
            <span
              className={`h-0.5 bg-white/80 rounded-full transition-all ${isMenuOpen ? "w-6 -rotate-45 -translate-y-2.5" : "w-5"}`}
            ></span>
          </div>
        </button>
